...
function safeExportBank(subject,ch,items){
 const chapter=S.data?.[subject]?.chapters?.find(x=>Number(x.number)===ch),v=vals(subject,ch),student=typeof currentStudent==='function'?currentStudent():{name:'Liam DeVries',grade:9};
 const title=`${subjectLabel(subject)} Chapter ${ch} - Completed PDF Study Work`;
 const rows=items.map((x,i)=>`${i+1}. [PDF page ${x.page}]${x.required?'':' [Optional]'} ${x.q}\nAnswer: ${clean(v[x.id])||'(No response saved)'}`).join('\n\n');
 const text=`Student: ${student.name}\nGrade: ${student.grade}\nCourse: ${subjectLabel(subject)}\nChapter ${ch}: ${chapter?.title||''}\n\n${rows}`;
 // Primary path: the app's academic PDF generator.
 try{
   if(typeof academicPdfBlob==='function'&&typeof download==='function'){
     const blocks=[{type:'p',text:`Student: ${student.name}`},{type:'p',text:`Grade: ${student.grade}`},{type:'p',text:`Course: ${subjectLabel(subject)}`},{type:'p',text:`Chapter ${ch}: ${chapter?.title||''}`},{type:'p',text:'Completed PDF Study Mode work. Source PDF page references are retained.'}];
     let last='';items.forEach((x,i)=>{if(x.section!==last){last=x.section;blocks.push({type:'h2',text:x.section})}blocks.push({type:'p',text:`${i+1}. [PDF page ${x.page}]${x.required?'':' [Optional]'} ${x.q}`});blocks.push({type:'p',text:clean(v[x.id])||'(No response saved)'})});
     const blob=academicPdfBlob({subject:subjectLabel(subject),chapter:ch,chapterTitle:chapter?.title||'',docTitle:title,dateLabel:'Exported',blocks});
     if(blob){download(blob,`${student.name.replace(/[^A-Za-z0-9]+/g,'_')}_${subjectLabel(subject).replace(/[^A-Za-z0-9]+/g,'_')}_Chapter_${String(ch).padStart(2,'0')}_PDF_Study.pdf`);return;}
   }
 }catch(err){console.error('Chapter '+ch+' PDF export failed; using print fallback',err)}
 // Reliable Safari/iPad fallback: open a printable document instead of doing nothing.
 const w=window.open('','_blank');
 if(!w){alert('The PDF export window was blocked. Allow pop-ups for this site, then tap Export completed PDF again.');return;}
 w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${esc(title)}</title><style>body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Arial,sans-serif;max-width:850px;margin:32px auto;padding:0 24px;line-height:1.45;color:#172033}h1{font-size:24px}h2{font-size:18px;margin-top:28px;border-bottom:1px solid #ddd;padding-bottom:5px}.q{margin:16px 0}.ans{white-space:pre-wrap;background:#f6f8fa;padding:10px;border-radius:6px}@media print{button{display:none}body{margin:0;max-width:none}}</style></head><body><button onclick="window.print()">Print / Save as PDF</button><h1>${esc(title)}</h1><p>${esc(`Student: ${student.name} | Grade: ${student.grade}`)}</p><p>${esc(`Chapter ${ch}: ${chapter?.title||''}`)}</p>${items.map((x,i)=>`<div class="q"><strong>${i+1}. [PDF page ${x.page}] ${esc(x.q)}</strong><div class="ans">${esc(clean(v[x.id])||'(No response saved)')}</div></div>`).join('')}</body></html>`);
 w.document.close();setTimeout(()=>{try{w.focus();w.print()}catch(_){}},250);
}
function exportBankLegacy(subject,ch,items){const chapter=S.data?.[subject]?.chapters?.find(x=>Number(x.number)===ch),v=vals(subject,ch),student=typeof currentStudent==='function'?currentStudent():{name:'Liam DeVries',grade:9};const blocks=[{type:'p',text:`Student: ${student.name}`},{type:'p',text:`Grade: ${student.grade}`},{type:'p',text:`Course: ${subjectLabel(subject)}`},{type:'p',text:`Chapter ${ch}: ${chapter?.title||''}`},{type:'p',text:'PDF Study Mode — questions extracted from the supplied chapter PDF. Optional activities are identified separately.'}];let last='';items.forEach((x,i)=>{if(x.section!==last){last=x.section;blocks.push({type:'h2',text:x.section})}blocks.push({type:'p',text:`${i+1}. [PDF page ${x.page}]${x.required?'':' [Optional]'} ${x.q}`});blocks.push({type:'p',text:clean(v[x.id])||'(No response saved)'})});if(typeof academicPdfBlob==='function'&&typeof download==='function'){const blob=academicPdfBlob({subject:subjectLabel(subject),chapter:ch,chapterTitle:chapter?.title||'',docTitle:`Chapter ${ch} PDF Study Mode — Completed Work`,dateLabel:'Exported',blocks});download(blob,`${student.name.replace(/[^A-Za-z0-9]+/g,'_')}_${subjectLabel(subject).replace(/[^A-Za-z0-9]+/g,'_')}_Chapter_${String(ch).padStart(2,'0')}_PDF_Study.pdf`)}else window.print()}
async function patch(){if(S.view!=='chapter'||!ACTIVE.has(Number(S.chapter))||!['biology','geography'].includes(S.subject))return;const overlay=document.getElementById('pdfstudy-overlay'),host=document.getElementById('pdfstudy-questions');if(!overlay||!host||overlay.dataset.src789==='1')return;overlay.dataset.src789='loading';const subject=S.subject,ch=Number(S.chapter);host.innerHTML='<div class="src789-loading">Loading the extracted Chapter '+ch+' source questions…</div>';try{const bank=await loadBank(subject,ch);if(!bank.items.length)throw new Error('No usable source questions were extracted');overlay.dataset.src789='1';host.innerHTML=render(subject,ch,bank.items);host.addEventListener('input',e=>{const t=e.target;if(!t.matches('textarea[data-src-id]'))return;saved[sk(subject,ch)]=saved[sk(subject,ch)]||{};saved[sk(subject,ch)][t.dataset.srcId]=t.value;saved[sk(subject,ch)+'-updatedAt']=new Date().toISOString();save();const s=sync(subject,ch,bank.items),pct=s.total?Math.round(s.answered/s.total*100):0,p=host.querySelector('.pdfstudy-progress');if(p)p.innerHTML=`<strong>${s.answered}/${s.total} required questions saved</strong><span>${pct}%</span><div><i style="width:${pct}%"></i></div>`});const old=document.getElementById('pdfstudy-export');if(old){const b=old.cloneNode(true);old.replaceWith(b);b.onclick=()=>safeExportBank(subject,ch,bank.items)}sync(subject,ch,bank.items);document.dispatchEvent(new CustomEvent('pdfstudy:bank-ready'));}catch(e){overlay.dataset.src789='error';host.innerHTML=`<div class="src789-error"><strong>Could not load the extracted Chapter ${ch} question bank.</strong><br>${esc(e.message)}</div>`;console.error(e)}}
document.addEventListener('click',e=>{if(e.target?.closest?.('#pdfstudy-open'))setTimeout(patch,0)},false);
document.addEventListener('pdfstudy:opened',()=>{setTimeout(patch,0)});
