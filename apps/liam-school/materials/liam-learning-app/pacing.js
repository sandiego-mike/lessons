(function(global){
  ...
  const SUBJECT_DEFAULTS={biology:{weekday:2,label:'Biology Day'},geography:{weekday:4,label:'World Geography Day'},math:{weekday:1,label:'Integrated Math I Day'},math3:{weekday:3,label:'Integrated Math III Day'}};
  ...
  function semesterTargets(subject){return subject==='biology'?{1:'Chapters 1-8',2:'Chapters 9-15 and 17'}:subject==='math'?{1:'Chapters 1-6',2:'Chapters 7-12'}:subject==='math3'?{1:'Units 1-5',2:'Units 6-9'}:{1:'Approximately chapters 1-17',2:'Approximately chapters 18-34'}}
  function scheduledUnits(subject,course,saved={},options={}){
    ...
    for(const sem of [1,2]){
      ...
      let target=subject==='biology'
        ...
        :(subject==='math'||subject==='math3')
          ?(subject==='math3'?(sem===1?chapters.filter(c=>c.number<=5):chapters.filter(c=>c.number>=6)):(sem===1?chapters.filter(c=>c.number>=1&&c.number<=6):chapters.filter(c=>c.number>=7&&c.number<=12)))
          :(sem===1?chapters.filter(c=>c.number>=1&&c.number<=17):chapters.filter(c=>c.number>=18&&c.number<=34));
      let i=0;
      if(subject==='math'||subject==='math3'){
