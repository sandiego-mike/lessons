...
function openMode(){
  if(!SUBJECTS.has(S.subject))return;
  let ch,questions,subject,url,vals;
  try{
    ch=chap(); if(!ch)return;
    subject=S.subject; questions=collectQuestions(ch); url=pdfUrl(subject,ch.number); vals=answersFor(subject,ch.number);
    const compactSafari=window.matchMedia&&window.matchMedia('(max-width: 850px)').matches;
    const frameUrl=compactSafari?'about:blank':url+'#view=FitH';
    syncCompletion(subject,ch,questions,vals);
    closeMode();
    document.body.insertAdjacentHTML('beforeend',`<div id="pdfstudy-overlay" class="pdfstudy-overlay" data-pane="questions"><div class="pdfstudy-shell"><header><div><span>Alternate learning path</span><h2>${escHtml(subjectLabel(subject))} · Chapter ${ch.number}: ${escHtml(ch.title)}</h2><p>Read the supplied chapter PDF and answer the chapter questions. Responses save automatically.</p></div><div class="pdfstudy-actions"><a href="${url}" target="_blank" rel="noopener">Open chapter PDF</a><button id="pdfstudy-export" type="button">Export completed PDF</button><button id="pdfstudy-close" type="button">Close</button></div><div class="pdfstudy-mobile-switch"><button type="button" data-pane="pdf">📖 PDF</button><button type="button" data-pane="questions" class="active">✍️ Questions</button></div></header><main><div class="pdfstudy-reader"><iframe src="${frameUrl}" title="Chapter ${ch.number} PDF"></iframe><div class="pdfstudy-mobile-reader-fallback"><p>Open the chapter textbook in a separate tab, then return here to answer.</p><a href="${url}" target="_blank" rel="noopener">Open Chapter ${ch.number} PDF</a><small>Your responses remain saved here.</small></div></div><aside id="pdfstudy-questions">${renderQuestions(subject,ch,questions)}</aside></main></div></div>`);
  }catch(err){console.error('PDF Study Mode open failed:',err);return;}
...
function injectButton(){
  ...
  const b=document.createElement('button');b.id='pdfstudy-open';b.type='button';b.className='tab pdfstudy-tab';b.textContent='📖 PDF Study Mode';
  tabs.appendChild(b);
}
...
// Keep PDF Study Mode passive on Safari: no document-wide MutationObserver.
// The app's normal navigation clicks get one lightweight deferred button check.
document.addEventListener('click',e=>{
  const pdfButton=e.target?.closest?.('#pdfstudy-open');
  if(pdfButton){e.preventDefault();e.stopPropagation();openMode();return;}
  const target=e.target?.closest?.('[data-subject],button,a');
  if(target)setTimeout(()=>{try{injectButton()}catch(err){console.error('PDF Study Mode button:',err)}},0);
},true);
