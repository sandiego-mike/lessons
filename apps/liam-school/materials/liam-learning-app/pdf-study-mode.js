...
function collectQuestions(ch){
  const out=[],seen=new Set(),guide=guideFor(ch);
  (guide?.steps||[]).forEach((st,si)=>{
    if(st?.vocabularyCheck)return;
    if(st.try)addQuestion(out,seen,st.try,'Guided lesson',`guide-${si}-try`,st.tryAnswer||st.more);
    (st.questions||[]).forEach((q,qi)=>addQuestion(out,seen,q,'Guided mastery',`guide-${si}-q${qi}`,st.more));
    if(st.apply&&!st.noApply)addQuestion(out,seen,st.apply,'Application',`guide-${si}-apply`,st.more);
  });
  const worksheet=worksheetItems(ch)||[];
  worksheet.forEach((q,i)=>addQuestion(out,seen,q.prompt||q.question,'Worksheet',`worksheet-${i}`,q.answer));
  const checks=knowledgeItems(ch)||[];
  checks.forEach((q,i)=>addQuestion(out,seen,q.prompt||q.question,'Knowledge check',`check-${i}`,q.answer));
  (ch.worksheet||[]).forEach((q,i)=>addQuestion(out,seen,q.prompt||q.question,'Textbook question',`source-${i}`,q.answer));
  try{if(typeof textbookSectionQuestions==='function')(textbookSectionQuestions(ch,99)||[]).forEach((q,i)=>addQuestion(out,seen,q.prompt||q.question,'Textbook review',`textbook-${i}`,q.answer));}catch(_){}
  return out;
}
...
function openMode(){
  ...
  closeMode();document.body.insertAdjacentHTML('beforeend',`<div id="pdfstudy-overlay" class="pdfstudy-overlay"><div class="pdfstudy-shell"><header><div><span>Alternate learning path</span><h2>${escHtml(subjectLabel(subject))} · Chapter ${ch.number}: ${escHtml(ch.title)}</h2><p>Read the actual supplied chapter PDF on the left. Answer the chapter questions on the right. Every response saves automatically and also feeds the main course progress.</p></div><div class="pdfstudy-actions"><a href="${url}" target="_blank" rel="noopener">Open PDF separately</a><button id="pdfstudy-export">Export completed PDF</button><button id="pdfstudy-close">Close</button></div></header><main><div class="pdfstudy-reader"><iframe src="${url}#view=FitH" title="Chapter ${ch.number} PDF"></iframe></div><aside id="pdfstudy-questions">${renderQuestions(subject,ch,questions)}</aside></main></div></div>`);
