...
const bio5=vm.runInContext(`(()=>{S.subject='biology';S.chapter=5;let ch=chap(),items=worksheetItems(ch);return {prompts:items.map(x=>x.prompt),html:worksheet(ch)}})()`,context);
assert.equal(bio5.prompts.length,15,'Biology Chapter 5 needs 15 distinct available practice questions');
assert.doesNotMatch(bio5.prompts.join(' '),/Section 5\.1|biotic|abiotic|pond parts|food chain|symbiosis/i,'Biology Chapter 5 worksheet must not reuse Chapter 2 ecology material or broken section labels');
assert.match(bio5.prompts.join(' '),/habitat fragmentation/i,'Biology Chapter 5 worksheet must practice the rewritten conservation lesson');
assert.match(bio5.html,/Focused practice/,'Biology worksheets must identify the focused core practice');
assert.match(bio5.html,/Optional extra practice \(9 questions\)/,'Biology worksheets must collapse the nine optional questions');
